...
# ============================================================
# Comparison of our evolution of k = 0.1 Mpc^-1
# ============================================================
k   = 0.1
sol = solve_perturbations_tca(k)
tau = np.logspace(np.log10(sol.t[0] * 1.05),
                  np.log10(bg['tau0'] * 0.999), 600)


# our expectation for when TCA starts to break down
lo, hi = sol.t[0], bg['tau0']
while hi - lo > 1e-6:
    mid = 0.5 * (lo + hi)
...
ours    = our_perts(sol, tau)
camb_d  = camb_perts(k, tau)

fig, axes = plt.subplots(3, 1, figsize=(5, 5.5), sharex=True)
for ax, key, label in zip(axes,
                          ['d_g', 'd_c', 'd_b'],
                          [r'$d_\gamma$', r'$d_c$', r'$d_b$']):
    ax.plot(tau, camb_d[key], 'k',   lw=1.2, label='CAMB')
    ax.plot(tau, ours[key],   'r--', lw=1.0, label='Ours (TCA)')
    ax.set_xscale('log')
    ax.set_xlim(tau[0], tau[-1])
    ax.set_ylabel(label)

    ax.axvspan(tau_switch, tau[-1], facecolor='lightgrey', alpha=0.5, zorder=-1)
    

axes[0].set_title(r'$k = %.1f\ {{\rm Mpc}}^{{-1}}$'%(k), fontsize=11)
axes[0].legend(frameon=False, fontsize=10, loc='lower left')
axes[-1].set_xlabel(r'$\tau$ [Mpc]')
axes[1].set_ylim(0, 15)
axes[2].set_ylim(-2.3, 4.3)

plt.savefig('figures/section06_TCA.pdf', bbox_inches='tight')
plt.close()
